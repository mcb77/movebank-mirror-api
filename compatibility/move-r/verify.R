...
# ── URL override ───────────────────────────────────────────────────────────
# Source the shim that monkey-patches httr::GET so `move`'s hardcoded
# Movebank URL gets rewritten to our replay server. See url_shim.R for the
# mechanism and why it's needed. Run this script from its own directory so
# the relative source() resolves.
source("url_shim.R")
